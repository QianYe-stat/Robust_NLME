...
"
  ...
  for(k in 1:rep){
    ...
    list(NLME.out=NLME.out,TS.out=TS.out, JM.out=JM.out, alpha.NLME=alpha.NLME)
  
}

reg = makeRegistry(file.dir = here::here("Batch","registry"), seed = 123)
batchMap(fun = Simulation , rep = rep(10, 10))
submitJobs(resources = list(walltime = Inf , memory = Inf))
loadResult(1)
getStatus()
